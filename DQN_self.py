...
#gameName = 'breakout_atari' # the name of the game being played for log files
#gameName = 'Breakout' # the name of the game being played for log files
#actionNum = 18 # number of valid actions
...
FRAME_PER_ACTION = 1
TH_L = 150 #lower bound for cv.threshold
TH_H = 220 #Upper bound for cv.threshold
...
def createNetwork(actionNum):
    ...
    W_fc2 = weight_variable([512, actionNum])
    b_fc2 = bias_variable([actionNum])
...
def trainNetwork(s, readout, h_fc1, sess, gameName, gamePath, actionNum):
    ...
    # printing
    a_file = open("log_files/logs_" + gameName + "_readout.txt", 'w+')
    h_file = open("log_files/logs_" + gameName + "_hidden.txt", 'w+')

    # get the first state by doing nothing and preprocess the image to 80x80x4
    do_nothing = np.zeros(actionNum)
    do_nothing[0] = 1
    x_t, r_0, terminal = game_state.frame_step(do_nothing)
    x_t = cv2.cvtColor(cv2.resize(x_t, (80, 80)), cv2.COLOR_BGR2GRAY)
    #ret, x_t = cv2.threshold(x_t,TH_L,TH_H,cv2.THRESH_BINARY)
    ...
    while True:
        ...
        if t % FRAME_PER_ACTION == 0:
            if random.random() <= epsilon:
                ...
                a_t[random.randrange(actionNum)] = 1
            else:
...
def playGame(gameName):
    ...
    trainNetwork(s, readout, h_fc1, sess, gameName, gamePath, actionNum)
